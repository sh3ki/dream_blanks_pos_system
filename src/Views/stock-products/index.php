...
$activeTab = $active_tab ?? 'stock-products';
...
function spSortLink(string $col, string $label, string $currentSort, string $currentOrder, array $filters): string {
    ...
    $q = http_build_query(array_merge($filters, ['tab' => 'stock-products', 'sort' => $col, 'order' => $nextOrder]));
...
function roSortLinkSp(string $col, string $label, string $cs, string $co, array $rf): string {
...
<div class="stats-grid">
  <?php
    function spCard(string $label, string $icon, string $value, string $valueColor, string $sub, string $href, bool $isActive): string {
    ...
    $spUrl = fn(string $s) => '?tab=stock-products&stock_status=' . $s;
    $roUrl = fn(string $s) => '?tab=restock&restock_status=' . $s;
  ?>
  <?= spCard('Low Stock Items',  'alert',   (string)(int)($sp_stats['low_stock_count']    ?? 0), 'var(--color-warning)', 'Needs attention',       $spUrl('low_stock'),   $activeTab === 'stock-products' && $stockStatus === 'low_stock') ?>
  <?= spCard('Out of Stock',     'alert',   (string)(int)($sp_stats['out_of_stock_count'] ?? 0), 'var(--color-danger)',  'Zero quantity',          $spUrl('out_of_stock'), $activeTab === 'stock-products' && $stockStatus === 'out_of_stock') ?>
  <?= spCard('In Stock',         'package', (string)(int)($sp_stats['in_stock_count']     ?? 0), 'var(--color-success)', 'Sufficient stock',       $spUrl('in_stock'),     $activeTab === 'stock-products' && $stockStatus === 'in_stock') ?>
  <?= spCard('Pending Restock',  'alert',   (string)(int)($restock_stats['pending_count']   ?? 0), 'var(--color-primary)', 'Awaiting delivery',   $roUrl('ordered'),    $activeTab === 'restock' && $restockStatus === 'ordered') ?>
  <?= spCard('Delivered Orders', 'check',   (string)(int)($restock_stats['delivered_count'] ?? 0), 'var(--color-success)', 'Successfully received', $roUrl('delivered'), $activeTab === 'restock' && $restockStatus === 'delivered') ?>
</div>

<!-- Tabs -->
<div style="display:flex;gap:0;border-bottom:2px solid var(--color-gray-100);margin-bottom:16px">
  <button id="tab-sp-btn" onclick="switchSpTab('stock-products')"
    style="padding:10px 20px;border:none;background:none;cursor:pointer;font-weight:600;font-size:.9rem;border-bottom:2px solid <?= $activeTab === 'stock-products' ? 'var(--color-primary)' : 'transparent' ?>;color:<?= $activeTab === 'stock-products' ? 'var(--color-primary)' : 'var(--color-gray-500)' ?>;margin-bottom:-2px">
    Stock Products
  </button>
  <button id="tab-hist-btn" onclick="switchSpTab('history')"
  ...
  <button id="tab-restock-btn" onclick="switchSpTab('restock')"
...
<div class="card">
    ...
    <table class="data-table">
      <thead>
        <tr style="cursor:pointer">
          ...
          <th style="padding:0"><?= spSortLink('stock_status',   'Stock Status',  $sort, $order, $filters) ?></th>
  ...
  <?php
    $spPqFilters = array_filter(array_merge($filters, ['tab' => 'stock-products', 'page' => null, 'per_page' => null]), fn($v) => $v !== null && $v !== '');
    echo renderPagination($pagination, $spPqFilters);
  ?>
  </div><!-- /spResultsContainer -->
</div>
</div><!-- /tab-stock-products -->
      ...
      <div class="form-group">
        ...
        <div style="display:flex;gap:12px">
          <label style="display:flex;align-items:center;gap:6px;cursor:pointer">
            <input type="radio" name="adjustType" value="add" checked onchange="recalcAdjust()"> Add
          </label>
          <label style="display:flex;align-items:center;gap:6px;cursor:pointer">
            <input type="radio" name="adjustType" value="deduct" onchange="recalcAdjust()"> Deduct
          </label>
        </div>
      </div>
      <div class="form-group">
        <label class="form-label">Quantity <span class="required">*</span></label>
        <input type="number" id="adjustQty" class="form-input" min="1" value="1" oninput="recalcAdjust()">
      </div>
      <div class="form-group">
        <label class="form-label">Reason</label>
        <input type="text" id="adjustReason" class="form-input" placeholder="e.g., Manual correction" value="Manual adjustment">
      </div>
      <div style="background:var(--color-gray-50);border:1px solid var(--color-gray-100);border-radius:8px;padding:10px 14px;display:flex;justify-content:space-between;align-items:center">
        <span style="font-size:.85rem;color:var(--color-gray-500)">New Quantity after adjustment:</span>
        <strong id="adjustNewQty" style="font-size:1.1rem">—</strong>
      </div>
    </div>
    <div class="modal-footer">
...
let _currentAdjustQty = 0;

function adjustSp(id, name, currentQty) {
  _currentSpId = id;
  _currentAdjustQty = parseInt(currentQty, 10) || 0;
...
function recalcAdjust() {
  ...
  const el   = document.getElementById('adjustNewQty');
  if (!el) return;
  if (!qty || qty <= 0) { el.textContent = '—'; el.style.color = ''; return; }
  const newQty = type === 'deduct' ? _currentAdjustQty - qty : _currentAdjustQty + qty;
  el.textContent = newQty;
  el.style.color = newQty < 0 ? 'var(--color-danger)' : (newQty === 0 ? 'var(--color-warning)' : 'var(--color-success)');
}
...
function switchSpTab(tab) {
  document.getElementById('tab-stock-products').style.display = tab === 'stock-products' ? 'block' : 'none';
  document.getElementById('tab-history').style.display        = tab === 'history'         ? 'block' : 'none';
  document.getElementById('tab-restock').style.display        = tab === 'restock'         ? 'block' : 'none';
  const tabs = { 'stock-products': 'tab-sp-btn', 'history': 'tab-hist-btn', 'restock': 'tab-restock-btn' };
  ...
  qs.set('tab', tab);
  // clear cross-tab params
  if (tab !== 'stock-products') { qs.delete('stock_status'); }
...
function applyHistFilters() {
  ...
  params.set('tab', 'history');
  const url = window.location.origin + spBaseUrl + '?' + params.toString();
...
async function submitRestock() {
  const items = Object.entries(_spSels).map(([id, qty]) => ({ stock_product_id: parseInt(id), quantity_ordered: parseInt(qty) }));
  ...
  try {
    ...
    if (data.success) {
      ...
      setTimeout(() => { switchSpTab('restock'); window.location.href = spBaseUrl + '?tab=restock'; }, 600);
    } else showToast(data.message || 'Failed', 'error');
  } catch (e) { showToast('Network error', 'error'); }
  ...
  try {
    ...
    document.getElementById('vrmoBody').innerHTML = `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:16px">
        <div><div style="font-size:.75rem;color:var(--color-gray-400);font-weight:600;text-transform:uppercase">Order #</div><code>${esc(o.order_number || '-')}</code></div>
        <div><div style="font-size:.75rem;color:var(--color-gray-400);font-weight:600;text-transform:uppercase">Date</div>${o.order_date ? new Date(o.order_date).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}) : '-'}</div>
        <div><div style="font-size:.75rem;color:var(--color-gray-400);font-weight:600;text-transform:uppercase">Supplier</div>${esc(o.supplier_name || '-')}</div>
        <div><div style="font-size:.75rem;color:var(--color-gray-400);font-weight:600;text-transform:uppercase">Delivery Status</div>
        ...
        <div><div style="font-size:.75rem;color:var(--color-gray-400);font-weight:600;text-transform:uppercase">Expected Delivery</div>${o.expected_delivery_date ? new Date(o.expected_delivery_date).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}) : '-'}</div>
        <div><div style="font-size:.75rem;color:var(--color-gray-400);font-weight:600;text-transform:uppercase">Created By</div>${esc(o.created_by_name || '-')}</div>
      </div>
